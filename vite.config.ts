...
export default defineConfig({
  ...
  server: {
    port: process.env.PORT ? Number(process.env.PORT) : 5173,  // Convertimos el valor a un número
  ...
  preview: {
    port: process.env.PORT ? Number(process.env.PORT) : 4173,  // Convertimos el valor a un número
